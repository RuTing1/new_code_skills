...
#         basic_path = os.path.dirname(os.path.abspath(__file__))
        ...
        if orgin_data_path is None:
            orgin_data_path = "origin_data.csv"
...
class Deeplearing():
    ...
    def stock_lstm(self,):
        ...
        test_csv_path = os.path.join("test_data.csv")
        #学习率
        learning_rate = 0.001
...
#         def lstm_cell():
#             cell = tf.nn.rnn_cell(cell_num, reuse = tf.get_variable_scope().reuse)
#             return rnn.DropoutWrapper(cell, output_keep_prob = keep_prob)
        #这一行就是tensorflow定义多层LSTM网络的代码
        lstm_layers = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.BasicLSTMCell(cell_num) for _ in range(layer_num)], state_is_tuple = True)
        #初始化LSTM网络
        ...
        with tf.Session(config = config) as sess:
            ...
            for i in range(20000):
                ...
                if (i + 1) % 20 == 0:
                    ...
                    with open (os.path.join("show_y_pre.txt"), "w") as f:
        ...
        def stock_lstm_softmax(self,):
            ...
            model_save_path = r"\stock_rnn_save.ckpt"#os.path.join(basic_path, "stock_rnn.ckpt")
            #定义训练集的文件路径，当前为运行的python文件路径下，文件名为train_data.csv
            train_csv_path = os.path.join("train_data.csv")
            #定义测试集的文件路径，当前为运行的python文件路径下，文件名为test_data.csv
            test_csv_path = os.path.join("test_data.csv")
    ...
    #         def lstm_cell():
    #             cell = tf.nn.rnn_cell(cell_num, reuse=tf.get_variable_scope().reuse)
    #             return tf.nn.DropoutWrapper(cell, output_keep_prob=keep_prob)
            #这一行就是tensorflow定义多层LSTM网络的代码
            #lstm_layers = tf.nn.rnn_cell.MultiRNNCell([lstm_cell() for _ in range(layer_num)], state_is_tuple = True)
            lstm_layers = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.BasicLSTMCell(cell_num) for _ in range(layer_num)], state_is_tuple = True)
